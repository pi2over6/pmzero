...
pub type Members = BTreeMap<usize, String>;
...
pub fn save_games(games: &Vec<Game>) -> Result<(), std::io::Error> {
    std::fs::write(get_games_path(), serde_json::to_string_pretty(games)?)
}

pub fn save_members(members: &Members) -> Result<(), std::io::Error> {
    std::fs::write(get_members_path(), serde_json::to_string_pretty(members)?)
}
